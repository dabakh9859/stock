...
class DebtNotifier:
    ...
    def _send_notification(self, db: Session, client_id: int, data: dict):
        ...
        print(f"[DebtNotifier] DEBUG: channel='{channel}' dry_run={self._dry_run} client_id={client_id}")
        if self._dry_run:
        ...
        if channel == "email" and cl.email and os.getenv("SMTP_HOST"):
            ...
            # Utiliser n8n + WhatsApp
            to_phone = (cl.phone or '').strip()
            to_phone = self._normalize_phone(to_phone)
            if not to_phone:
                print(f"[DebtNotifier] No phone for client_id={client_id}, cannot send WhatsApp")
                return
            ok = self._send_whatsapp_n8n(to_phone, body, client_id)
            if ok:
                self._mark_sent(db, client_id)
            else:
                print(f"[DebtNotifier] Failed to send WhatsApp to {to_phone} (client_id={client_id})")
        elif channel == "sms":
    ...
    def _send_whatsapp_n8n(self, to_phone: str, body: str, client_id: int = None) -> bool:
        ...
        if not to_norm:
            print(f"[DebtNotifier] Cannot normalize phone: {to_phone}")
            return False
        ...
        try:
            ...
            with _urlrequest.urlopen(req, timeout=30) as resp:
                ...
                if ok:
                    print(f"[DebtNotifier] WhatsApp sent to {to_norm} (client_id={client_id})")
                else:
                    print(f"[DebtNotifier] WhatsApp send failed for {to_norm}: {resp_body.get('error', 'unknown')}")
                return ok
        except Exception as e:
            print(f"[DebtNotifier] WhatsApp send error: {e}")
            return False
